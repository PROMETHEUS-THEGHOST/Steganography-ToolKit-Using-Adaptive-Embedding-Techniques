...
def encode_txt_data(text, key):
    l = len(text, key)  # Adjust this line according to your existing logic
    i = 0
    add = ''
    while i < l:
        t = ord(text[i])
...
def txt_steg():
    while True:
        ...
        if choice1 == 1:
            ...
            encode_txt_data(text, key)
        elif choice1 == 2:
            key = input("Enter the decryption key: ")
            decode_txt_data(text, key) 
        elif choice1 == 3:
            break
        else:
...
def encode_img_data(img, key):
    ...
    print("\nEncoded the data successfully in the Image, and the image is saved with the name ", nameoffile)
...
def img_steg():
    while True:
        ...
        if choice1 == 1:
            ...
            print("Incorrect Choice")
        print("\n")
def encode_aud_data(key):
    import wave

    nameoffile = input("Enter the name of the file (with extension): ")
...
def encode_vid_data():
    ...
    out = cv2.VideoWriter('stegovideo.mp4',fourcc, 25.0, size)
    max_frame=0;
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == False:
...
def decode_vid_data(frame_):
    cap = cv2.VideoCapture('stegovideo.mp4')
    max_frame=0;
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == False:
    ...
    n=int(input())
    vidcap = cv2.VideoCapture('stegovideo.mp4')
    frame_number = 0
    while(vidcap.isOpened()):
        frame_number += 1
        ret, frame = vidcap.read()
